...
app.post('/api/attendance/web', async (req, res) => {
  const { action, latitude, longitude, timestamp, name } = req.body;
  console.log(`📥 Attendance request: ${action} | ${name} | ${latitude}, ${longitude}`);

  if (!action || isNaN(latitude) || isNaN(longitude) || !name) {
    return res.status(400).json({ success: false, message: 'Invalid input.' });
  }

  try {
    // Load dynamic office locations
    ...
    const staffMember = staffRows.find(
      row => row.get('Name')?.trim().toLowerCase() === name.trim().toLowerCase() &&
      row.get('Active') === 'Yes'
    );
